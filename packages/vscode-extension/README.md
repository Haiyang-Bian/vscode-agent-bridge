...
`0.3.0` is an unpublished Windows x64 desktop candidate distributed by side-loaded VSIX. The package contains a standalone MCP server, so testers do not need Bun, Node.js or the source repository.
...
VS Code Agent Bridge 将 Codex 连接到本机 VS Code 的 IDE 原生状态，并提供可恢复的 Agent 实验会话。`0.3.0` 是暂未发布 Marketplace 的 Windows x64 候选版本；通过 VSIX 旁加载测试时不需要安装 Bun、Node.js 或克隆源码。
